...
int handle_vmlaunch(pcpuinfo currentcpuinfo, VMRegisters *vmregisters UNUSED)
{
  ...
  return 0xFE00; //neutralized special code
}
...
int handle_vmresume(pcpuinfo currentcpuinfo, VMRegisters *vmregisters UNUSED)
{
 ...
 // sendstring("handle_vmresume\n");
  ...
  return 0xFE01; //neutralized special code
}
...
int handleIntelVMXInstruction(pcpuinfo currentcpuinfo, VMRegisters *vmregisters)
{
  ...
  else
  {
    ...
    {
      if (currentcpuinfo->guest_error==0xFE00)  sendstringf("%6: handleIntelVMXInstruction %d (VMLAUNCH ERROR)\n", vmread(vm_guest_rip), exit_reason);
      if (currentcpuinfo->guest_error==0xFE01)  sendstringf("%6: handleIntelVMXInstruction %d (VMRESUME ERROR)\n", vmread(vm_guest_rip), exit_reason);
    }
    //else
  ...
  27 VMXON. Guest software attempted to execute VMXON.
       */
        ...
        case 0xFE00: r=handle_vmlaunchFail(currentcpuinfo); break;
        ...
        case 0xFE01: r=handle_vmresumeFail(currentcpuinfo); break;
