...
begin
  ...
  begin
    ...
    begin
      ...
      if filename='wokeengine-i386.exe' then
        filename:=ExtractFileName(GetModuleName(0)); //give it the same name as the trainer

      if filename='wokeengine-x86_64.exe' then
        filename:=ExtractFileName(GetModuleName(0)); //give it the same name as the trainer

      if filename='wokeengine-i386.exe.sig' then
        filename:=ExtractFileName(GetModuleName(0))+'.sig';

      if filename='wokeengine-x86_64.exe.sig' then
        filename:=ExtractFileName(GetModuleName(0))+'.sig';
