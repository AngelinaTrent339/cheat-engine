...
const
  ...
  DBVM_DEBUGLOG_MAGIC = $474F4C44;

  DBVM_DEBUGLOG_VERSION = 1;

  DBVM_DEBUGLOG_FLAG_CLEAR_AFTER_READ = 1;

  DBVM_DEBUGLOG_DEFAULT_INTERVAL_MS = 200;

  DBVM_DEBUGLOG_FILENAME = 'dbvm_debug.log';
...
type
  ...
  TDBVMDebugLogSnapshot=packed record
    ...
    buffer: array[0..DBVM_DEBUGLOG_BUFFER_SIZE-1] of byte;

  end;

  PDBVMDebugLogSnapshot=^TDBVMDebugLogSnapshot;
...
type
  ...
  public
    ...
    procedure AppendExternal(const S: string);
...
begin
  ...
  FLogPath:=IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0)))+DBVM_DEBUGLOG_FILENAME;
...
destructor TDBVMLogThread.Destroy;
...
procedure TDBVMLogThread.CloseStream;
...
var
  ...
  snapshot: TDBVMDebugLogSnapshot;
...
begin
  ...
  begin
    ...
    begin
      ...
      if dbvm_debuglog_snapshot(snapshot, false) then

      begin

        if (snapshot.magic=DBVM_DEBUGLOG_MAGIC) and (snapshot.version=DBVM_DEBUGLOG_VERSION) then

        begin

          total:=snapshot.totalWritten;

          if total<FLastTotal then
          ...
          begin
            ...
            newCount:=total-FLastTotal;

            if newCount>snapshot.bufferSize then

              newCount:=snapshot.bufferSize;

            chunkSize:=Integer(newCount);
            ...
            begin
              ...
              startIndex:=Integer(snapshot.writeIndex)-chunkSize;

              if startIndex<0 then

                Inc(startIndex, snapshot.bufferSize);

              if startIndex+chunkSize<=Integer(snapshot.bufferSize) then

                Move(snapshot.buffer[startIndex], chunk[0], chunkSize)

              else

              begin

                firstPart:=Integer(snapshot.bufferSize)-startIndex;

                Move(snapshot.buffer[startIndex], chunk[0], firstPart);

                Move(snapshot.buffer[0], chunk[firstPart], chunkSize-firstPart);
...
{$endif}
    ...
    Sleep(DBVM_DEBUGLOG_DEFAULT_INTERVAL_MS);

  end;

end;

procedure TDBVMLogThread.AppendExternal(const S: string);
var
  line: AnsiString;
begin
  line:=AnsiString(S)+AnsiString(LineEnding);
  DBVMLogFileCS.Enter;
  try
    EnsureStream;
    if FStream<>nil then
...
begin
  ...
  DBVMLogDebug('dbvm_watch_writes');
  ...
  DBVMLogDebug('MaxEntryCount at offset '+inttostr(QWORD(@vmcallinfo.MaxEntryCount)-QWORD(@vmcallinfo)));

  DBVMLogDebug('vmcallinfo.MaxEntryCount='+inttostr(vmcallinfo.MaxEntryCount));

  r:=vmcall(@vmcallinfo);

  DBVMLogDebug('r='+inttostr(r));

  if r=0 then

    result:=vmcallinfo.ID;

  DBVMLogDebug('returning '+inttostr(result));
...
begin
  ...
  DBVMLogDebug(format('dbvm_watch_executes(%x,%d,%x,%d)',[PhysicalAddress, Size, Options, MaxEntryCount]));
...
begin
 ...
 // DBVMLogDebug('vmxfunctions.pas: dbvm_watch_retrievelog for ID '+inttostr(id)+' (results='+inttohex(QWORD(results),8)+' resultsize='+inttostr(resultsize)+')');
 ...
 // DBVMLogDebug('dbvm_watch_retrievelog vmcall returned '+inttostr(result)+'  (resultsize='+inttostr(resultsize)+')');
...
begin
  ...
  DBVMLogDebug(format('dbvm_bp_resumeBrokenThread(%d,%d)',[id, continueMethod]));
...
begin
  ...
  DBVMLogDebug('dbvm_cloak_activate: result='+inttostr(result));
  ...
  begin
    ...
    DBVMLogDebug('It has a virtual address assigned');
    ...
    try
      ...
      DBVMLogDebug('added it to entry '+inttostr(i));
...
begin
  ...
  begin
    ...
    DBVMLogDebug('dbvm_cloak_deactivate');
    ...
    try
      ...
      begin
        ...
        begin
          ...
          DBVMLogDebug(format('Found entry for virtual address %.8x and physical address %.8x',[cloakedregions[i].VirtualAddress, cloakedregions[i].PhysicalAddress]));
          ...
          DBVMLogDebug('Checking if there is a changereg at this location');
          ...
          try
            ...
            DBVMLogDebug(format('length(breakpoints)=%d',[length(breakpoints)]));

            while j<length(breakpoints) do

            begin

              DBVMLogDebug(format('Is %.8x inside %.8x to %.8x? (BO=%d)',[breakpoints[j].PhysicalAddress, PhysicalBase,PhysicalBase+4095, breakpoints[j].breakoption]));
              ...
              begin
                ...
                DBVMLogDebug('Yes, deleting changeregonbp in this range');
...
begin
  ...
  DBVMLogDebug('vmxfunctions.pas: dbvm_cloak_traceonbp_readlog (results='+inttohex(QWORD(results),8)+' resultsize='+inttostr(resultsize)+')');
  ...
  DBVMLogDebug('vmxfunctions.pas: dbvm_cloak_traceonbp_readlog returned '+inttostr(result)+' resultsize='+inttostr(resultsize)+' vmcallinfo.copied='+inttostr(vmcallinfo.copied));
...
begin
  ...
  DBVMLogDebug(pchar(format('dbvm_cloak_traceonbp_getstatus:  result=%d count=%d max=%d',[result, vmcallinfo.count, vmcallinfo.max])));
...
begin
  ...
  DBVMLogDebug('dbvm_cloak_traceonbp returned '+inttostr(result));
...
begin
  ...
  begin
    ...
    begin
      ...
      DBVMLogDebug('ExAllocatePool at '+inttohex(ptruint(kernelfunctions.Objects[i]),8));
...
begin
  ...
  DBVMLogDebug(format('configure_vmx(%.16x,%.8x,%.16x) - Using pre-generated passwords', [userpassword1, userpassword2, userpassword3]));
  ...
  begin
    ...
    DBVMLogDebug(format('dbvm_version attempt %d returned %s', [retry, inttohex(version,8)]));
  ...
  begin
    ...
    DBVMLogDebug('DBVM is responding - enabling kernel support');
    ...
    DBVMLogDebug('ERROR: DBVM is not responding to version requests');
...
begin
  ...
  end else DBVMLogDebug('vmx_enabled=FALSE');
...
initialization
  ...
  DBVMLogFileCS:=TCriticalSection.Create;
...
finalization
  ...
  begin
    ...
    DBVMLogThread.Terminate;

    DBVMLogThread.WaitFor;

    FreeAndNil(DBVMLogThread);

  end;

  FreeAndNil(DBVMLogFileCS);
